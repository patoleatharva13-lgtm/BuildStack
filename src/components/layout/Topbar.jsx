    ...
    <header className="h-16 bg-white border-b border-slate-200 px-6 flex items-center justify-between relative">
      ...
      <div className="flex items-center gap-4">
        <div className="relative">
          <button onClick={() => setShowNotifications((value) => !value)} className="relative text-slate-500 hover:text-slate-700">
            <Bell size={20} />
            {unreadCount > 0 && <span className="absolute -top-1 -right-1 h-2.5 w-2.5 rounded-full bg-red-500" />}
          </button>
          {showNotifications && (
            <div className="absolute right-0 top-full mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-lg z-20">
              ...
              <ul className="max-h-80 overflow-auto">
                {notifications.map((item) => (
                  <li key={item.id} className={`px-4 py-3 border-b last:border-b-0 ${item.unread ? 'bg-slate-50' : 'bg-white'}`}>
